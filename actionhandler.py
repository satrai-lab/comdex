...
# Returns: None
def post_entity(data,my_area,broker,port,qos,my_loc,bypass_existence_check=0,client=None):
    ...
    if(bypass_existence_check==0):
        if (check_existence(broker,port,check_topic)!=False):
            message = "Entity with this id already exists, did you mean to patch?"
            print("Error " + message)
    ...
    if(bypass_existence_check==1):
        ...
        print(special_context_provider_broadcast)
    print("Entity posted successfully: " + id)
    client.loop_stop()
    if connected_here:
        client.disconnect()
    return id
...
def recreate_multiple_entities(messagez, query='', topics='', timee='', limit=2000, georel='', geometry='', coordinates='', geoproperty='', context_given=''):
    ...
    for single_entities in messages_by_id.values():
        entity = recreate_single_entity(single_entities, query, topics, timee, georel, geometry, coordinates, geoproperty, context_given)
...
def subscribe_for_advertisement_notification(broker, port, topics, qos, entity_type_flag, watched_attributes_flag, entity_id_flag, watched_attributes, true_id, expires=None, stop_event=None, notification_queue=None, child_processes=None):
    ...
    def on_message(client, userdata, msg):
        ...
        if msg.payload.decode() != '':
            ...
            for j in jobs_to_terminate.pop(topic2, []):
                print(j)
                _stop_process(j)
    ...
    while run_flag:
        time.sleep(0.1)
        if expires is not None and (time.perf_counter() - start) > expires:
            run_flag = False
        if stop_event is not None and stop_event.is_set():
            run_flag = False

    for jobs in jobs_to_terminate.values():
        for j in jobs:
            _stop_process(j)
            if j in child_processes:
                child_processes.remove(j)
...
def post_subscription(data, broker, port, qos, my_area="unknown_area", notification_queue=None):
    ...
    if not entity_type_flag and not watched_attributes_flag and not entity_id_flag:
        raise ValueError("Subscription without filter information")

    big_topic = my_area + '/Subscriptions/' + context + '/Subscription/LNA/' + sub_id
    ...
    if truetype == '':
        truetype = '+'
    trueid2 = true_id if true_id != '' else '#'

    check_top = []
    for z in area:
        if not singleidadvertisement:
            check_top.append("provider/+/+/" + z + '/' + context + '/' + truetype2)
        else:
            check_top.append("provider/+/+/" + z + '/' + context + '/' + truetype + '/' + trueid2)
...
def delete_entity_attr(entity_id, attr_name, broker, port, hlink='+', my_area="unknown_area"):
    if hlink != '+':
        hlink = hlink.replace("/", "§")
    top = my_area + '/entities/' + hlink + '/+/+/' + entity_id + '/' + attr_name
    clear_retained(broker, port, top)
...
def get_entities(broker, port, hlink='+', entity_type=None, entity_id=None, attrs=None,
    ...
    if hlink != '+':
        hlink = hlink.replace("/", "§")
    area = area or ['+']
    typee_multi = entity_type if entity_type is not None else [1]
    id = entity_id or '#'
    attrs = attrs or '#'
    q = q or ''
    georel = georel or ''
    ...
    for typee in typee_multi:
        ...
        for z in area:
            if not singleidadvertisement:
                check_top.append("provider/+/+/" + z + '/' + hlink + '/' + typee)
            else:
        ...
        for messg in messages_for_context:
            ...
            key = initial_topic[1] + initial_topic[2] + initial_topic[3]
            if key in context_providers_full:
                continue
            context_providers_full.append(key)
            if attrs != '#':
                for i in attrs:
                    eid = '+' if id == '#' else id
                    topic.append(initial_topic[3] + '/entities/' + hlink + '/' + typee + '/+/' + eid + '/' + i)
            else:
                if id == '#':
            ...
            if messages:
                ...
                results = recreate_multiple_entities(messages, q, attrs, timee=timee, limit=limit,
                                                     georel=georel, geometry=geometry, coordinates=coordinates,
                                                     geoproperty=geoproperty, context_given=context_for_reconstruction)
...
def batch_create(entities, broker, port, qos=0, my_area="unknown_area", my_loc="unknown_location"):
    ...
    for data in entities:
        if not singleidadvertisement:
            typee = str(data.get('type', 'error_no_type'))
            if typee in advertisement_exists:
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hc:f:b:p:l:q:H:A:",["command=","file=","broker_address=","port=","qos=","HLink=","singleidadvertisement="])
    ...
    #CREATION OF SUBSRIPTIONS        
        ...
        with open(file) as json_file:
            ...
            t = threading.Thread(
                target=subscribe_for_advertisement_notification,
                args=(broker, port, check_top, qos, entity_type_flag, watched_attributes_flag, entity_id_flag, watched_attributes, true_id),
                kwargs={"expires": sub_expires, "stop_event": stop_event},
                daemon=True
            )
            t.start()
            ...
            print(f"Subscription {id} registered.")
            t.join()
            ...
            if (Forwarding==1):    
                for messg in messages_for_context:
                    ...
                    if attrs!='#':
                        for i in attrs:
                            if id == '#':
                                top=initial_topic[3]+'/entities/'+HLink+'/'+typee+'/+/+/'+i
                            else:
                                top=initial_topic[3]+'/entities/'+HLink+'/'+typee+'/+/'+id+'/'+i
                            topic.append(top)
                    else:
                        if id == '#':
                            top=initial_topic[3]+'/entities/'+HLink+'/'+typee+'/#'
                        else:
